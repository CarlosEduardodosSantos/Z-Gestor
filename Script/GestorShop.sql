...
IF NOT EXISTS(SELECT NAME FROM SYSCOLUMNS WHERE NAME = 'Estacao' AND ID IN
     (SELECT ID FROM SYSOBJECTS WHERE NAME = 'Pendente_1'))
      Alter table Pendente_1 Add Estacao Varchar(100)
GO

IF NOT EXISTS(SELECT NAME FROM SYSCOLUMNS WHERE NAME = 'PagamentoOnline' AND ID IN
     (SELECT ID FROM SYSOBJECTS WHERE NAME = 'Pendente_1'))
      Alter table Pendente_1 Add PagamentoOnline Bit
GO
IF NOT EXISTS(SELECT NAME FROM SYSCOLUMNS WHERE NAME = 'ValorVoucher' AND ID IN
     (SELECT ID FROM SYSOBJECTS WHERE NAME = 'Pendente_1'))
      Alter table Pendente_1 Add ValorVoucher Decimal(18,2)
GO

IF NOT EXISTS(SELECT NAME FROM SYSCOLUMNS WHERE NAME = 'AppId' AND ID IN
     (SELECT ID FROM SYSOBJECTS WHERE NAME = 'Pendente_1'))
      Alter table Pendente_1 Add AppId Varchar(50)
GO
...
Begin
	...
	Set @clienteId = Isnull((Select Codigo From Cliente Where Fone1 = @fone),0)

	If(@clienteId = 0)
	Begin
		Insert Into Cliente(Fone1, Nome, ENDERECO, Bairro, Cep, Cidade, UF, CIC, OBS1) Values (
